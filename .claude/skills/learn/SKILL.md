...
- **先展示后确认**：任何要用户确认的产物（地图、设计、范围等）必须先在独立消息里完整呈现（关键产物渲染到浏览器），等用户看过再发确认型问题；禁止对用户尚未看到的内容用 AskUserQuestion 让其拍板
- **流程透明**：开题时先用一两句讲清「这套流程怎么走、会产出哪些东西」；元流程术语（范围、slug、知识地图、苏格拉底、gate 等）首次出现时挂一句人话解释，不假设用户已懂
